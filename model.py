...
class LayerStacks(nn.Module):
  ...
  def forward(self, x, ls_indices):
    ...
    l1x_ = l1c_ + l1f_
    # multiply sqr crelu result by (254/255) to match quantized version
    l1x_ = torch.clamp(torch.cat([torch.pow(l1x_, 2.0) * (254/255), l1x_], dim=1), 0.0, 1.0)
...
class NNUE(pl.LightningModule):
  ...
  def _forward_inner(self, us, them, wp, bp, psqt_indices, layer_stack_indices):
    ...
    l0_ = torch.cat(l0_s1, dim=1) * (254/255)
